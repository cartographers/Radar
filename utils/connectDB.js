...
const loadTableForeignKeys = (settings) => {
	...
	console.log('FK settings....', settings)
...
const joinTables = (settings) => {
	console.log('joinTables util func settings....', settings)
...
const loadFields = (settings) => {
	const postgresUrl = 'postgres://localhost:5432/' + settings.database
	const client = new pg.Client(postgresUrl)
	const joinTable = joinTables(settings)
	let jointTables = joinTable.join(' ').trim() || ''
