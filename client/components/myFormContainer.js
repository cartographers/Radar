  ...
  return <div>
    ...
    {props.selectThese.map((sel, index) => {
      return <div key={index}>
        <select key={index} onChange={props.handleChange.bind(this, index, 'selectThese')}
                value={props.selectThese[index]}>
          {props.columns && props.columns.map((val, i) => <option value={val} key={i}>{val}</option>)}
        </select>
        <button type="button" className="btn btn-danger" onClick={props.handleRemove.bind(this, index, 'selectThese')}>
          -
        </button>
      </div>
    })
  ...
  return <div>
    ...
    }
    {
      props.whereThese.map((sel, index) => {
        return <div key={index}>
          <select name="col" onChange={props.handleChange.bind(this, index, 'whereThese')}
                  value={props.whereThese[index].col}>
            {props.columns && props.columns.map((val, i) => <option value={val} key={i}>{val}</option>)}
          </select>
          <h4>is</h4>
          <select name="is" onChange={props.handleChange.bind(this, index, 'whereThese')}
                  value={props.whereThese[index].literal}>
          ...
          <button type="button" className="btn btn-danger" onClick={props.handleRemove.bind(this, index, 'whereThese')}>
            -
          </button>
        </div>
      })
            ...
            .map((graphInfo, index) => {
              return (
                <div className="col-lg-6">
                  <Well style={{width: 40 + 'em', height: 30 + 'em', marginRight: 1 + 'em'}}>
                    <Button onClick={props.handleChartDelete.bind(this, graphInfo)} bsStyle="danger">x</Button>
